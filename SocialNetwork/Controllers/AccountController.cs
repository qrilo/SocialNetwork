    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(UserLoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var confirmed = await _emailService.IsEmailConfirmed(loginResult.UserDTO.Email);
                    if (confirmed)
                    {
                        await Authenticate(loginResult.UserDTO);

                        return RedirectToAction("Index", "Profile", new { id = loginResult.UserDTO.Id.ToString() });
                    }
 ...
 //         model.Birthday = new DateTime(2000, 1, 1);
        ...
        public async Task<IActionResult> Register(UserRegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                dto = await _authService.TryRegisterUserAsync(dto);

                var token = await _emailService.GenerateToken(dto.Email);
                var callbackUrl = Url.Action(
                "ConfirmEmail",
                "Account",
                new { userId = dto.Id, code = token },
                protocol: HttpContext.Request.Scheme);

                await _emailService.SendEmail(dto.Email, "Confirm your account", 
                    $"Please confirm your email address by clicking on the link below: <a href='{callbackUrl}'>click here</a>");
        ...
        public async Task<IActionResult> ConfirmEmail(int userId, string code)
        {
            ...
            var confirmed = await _emailService.TryConfirmEmail(userId, code);
            if (confirmed)
            {
                return Content("NIceeeeee");
            }
